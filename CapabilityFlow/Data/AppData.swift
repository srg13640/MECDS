...
struct AppData {
    ...
    let processSteps: [ProcessStep] = [
        ...
        ProcessStep(
            ...
            detailedDescription: "The FCC Futures Integration Directorate (FID), FCD requirements writers, HQDA G-8 Force Development, and CAC/proponents translate evidence into formal requirements. The discipline here is writing the minimum viable requirement that the evidence justifies — not a wish list. Note on validation: under the JFRP (January 2026), the JROC no longer routinely validates Service-specific requirements — the Army validates its own through the AROC (per AR 71-9/ACIDS, which is pending post-JFRP update).",
            organizations: ["FCC FID", "FCD requirements writers", "HQDA G-8 FD", "CAC/Proponents"],
            outputs: ["Validated gaps", "CBAs / TBAs", "CDDs", "A-CDDs", "ICDs", "CONOPS", "Characteristics of Need (emerging)", "AROC validation decisions"],
        ...
        ProcessStep(
            ...
            detailedDescription: "ASA(ALT), PAEs, CPEs, PEOs, PMs, and contracting officials execute the acquisition. In the emerging model, portfolio accountability (PAEs/CPEs) sits alongside program execution (PEOs/PMs), and pathway selection is matched to the type of capability and how fast it must move. In November 2025 the broader system was redesignated the Warfighting Acquisition System (WAS), and the Army stood up six PAEs with PEOs transitioning to CPEs.",
    ...
    let organizations: [Organization] = [
        ...
        Organization(
            ...
            typicalOutputs: ["Doctrine", "Training products", "DOTMLPF-P assessments", "Force Design Updates (developed from FCC-approved FDWs)"],
        ...
        Organization(
            ...
            abbreviation: "PAE",
            category: .acquisition,
            plainEnglishRole: "Holds accountability for a portfolio of capabilities and balances trade-offs across it. Six PAEs were announced in November 2025 (Fires; Maneuver Ground; Maneuver Air; C2 / Counter-C2; Agile Sustainment and Ammo; Layered Protection and CBRND). PAEs dual-report: to ASA(ALT) for acquisition and materiel development, and to T2COM for requirements generation.",
            owns: ["Portfolio-level accountability", "Cross-program trade-offs", "Test integration within the portfolio"],
            doesNotOwn: ["Sole ownership of requirements (generated with T2COM/FCC)", "Resourcing decisions"],
            keyRelationships: ["ASA(ALT) (acquisition reporting)", "T2COM (requirements-generation reporting)", "CPEs", "PMs", "ATEC"],
        ...
        Organization(
            ...
            abbreviation: "CPE",
            category: .acquisition,
            plainEnglishRole: "Leads execution of a specific capability program area within a PAE portfolio. Beginning January 2026, PEOs began transitioning into CPEs under the capability-centric model.",
        ...
        Organization(
            ...
            abbreviation: "PEO",
            category: .acquisition,
            plainEnglishRole: "Executes a group of acquisition programs and supervises the PMs under it. Under the 2025 acquisition reform, PEOs are transitioning into the PAE/CPE portfolio structure (rollout began January 2026).",
    ...
    let glossary: [GlossaryTerm] = [
        ...
        GlossaryTerm(id: "fcw", term: "FCW", definition: "Future Capability Workbook — an early artifact in the Force Design Update (FDU) process. FCC develops FCWs to capture future capability needs, then adds granularity through experimentation as the work matures toward a Future Design Workbook (FDW). (Practitioner usage within FCC; not yet described in public regulation.)", whyItMatters: "It is where future-force design work actually starts inside FCC — well before anything formal exists downstream.", example: "The team captured the emerging capability in an FCW ahead of the next experimentation cycle."),
        GlossaryTerm(id: "fdw", term: "FDW", definition: "Future Design Workbook — the matured successor to a Future Capability Workbook in the FDU process. FCC refines FCWs through experimentation into FDWs; once FCC approves an FDW, it is handed off to the Combined Arms Command (CAC) for Force Design Update development. (Practitioner usage within FCC; not yet described in public regulation.)", whyItMatters: "FDW approval is the handoff point where future-force design moves from FCC to CAC for formal FDU development.", example: "Once FCC approved the FDW, CAC began developing the FDU."),
        GlossaryTerm(id: "fdu", term: "FDU", definition: "Force Design Update — a proposed change to how a formation is organized. In the emerging FCC process, FDUs grow from Future Capability Workbooks (FCWs) matured through experimentation into Future Design Workbooks (FDWs); FCC-approved FDWs hand off to CAC for FDU development.", whyItMatters: "It is the mechanism for organizational (non-materiel) change — and the end of the FCW → FDW → FDU pipeline.", example: "The team pursued an FDU to reorganize the unit rather than buy new equipment."),
        GlossaryTerm(id: "jcids", term: "JCIDS", definition: "Joint Capabilities Integration and Development System — the FORMER Joint process for identifying and validating capability requirements. Replaced in its entirety by the Joint Force Requirements Process (JFRP) on 15 January 2026.", whyItMatters: "You will still see JCIDS in older documents — and in AR 71-9, which has not yet been publicly updated. Previously validated JROC requirements remain valid.", example: "The legacy requirement was validated under JCIDS before the JFRP replaced it.", statutoryBasis: "CJCSI 5123.01J (15 Jan 2026) states the JFRP 'replaces the JCIDS in its entirety'; the 2021 JCIDS Manual was canceled. JROC authorities: 10 U.S.C. §181."),
        GlossaryTerm(id: "jfrp", term: "JFRP", definition: "Joint Force Requirements Process — the CURRENT Joint requirements process, which replaced JCIDS. It focuses the JROC on Joint Force Design, Joint Capability Integration, and combatant command requirements through Joint Operational Problems; validation of Service-specific requirements now rests with the Services.", whyItMatters: "It is the Joint front door for requirements — and it deliberately returns Service-level validation to the Services (for the Army, the AROC).", example: "The Joint-interoperable need entered the JFRP, while the Army validated its Service-specific requirement itself.", statutoryBasis: "CJCSI 5123.01J, 'Charter of the JROC and the Joint Force Requirements Process,' and CJCSM 5123.01, both effective 15 January 2026; JROC mission per 10 U.S.C. §181 as amended."),
        ...
        GlossaryTerm(id: "pae", term: "PAE", definition: "Portfolio Acquisition Executive — accountable for a portfolio of capabilities and trade-offs across it. Six PAEs were announced in November 2025: Fires; Maneuver Ground; Maneuver Air; Command and Control / Counter-C2; Agile Sustainment and Ammo; Layered Protection and CBRND.", whyItMatters: "PAEs dual-report — to ASA(ALT) for acquisition and materiel development, and to T2COM for requirements generation — bridging the requirements and acquisition worlds.", example: "The PAE balanced investment across the portfolio rather than program by program."),
        GlossaryTerm(id: "cpe", term: "CPE", definition: "Capability Program Executive — leads execution of a specific capability program area within a PAE portfolio. Beginning January 2026, PEOs began transitioning into CPEs under the capability-centric portfolio model.", whyItMatters: "It provides focused execution leadership under the portfolio — replacing the program-centric PEO construct.", example: "The CPE drove execution of the capability program."),
        ...
        GlossaryTerm(id: "acids", term: "ACIDS", definition: "Army Capabilities Integration and Development System — the Army's requirements process under AR 71-9 (Warfighting Capabilities Determination, 2021), originally built to implement JCIDS.", whyItMatters: "AR 71-9 remains active but predates the JFRP — the Army regulation is pending a post-JFRP update, so treat ACIDS terminology as in transition.", example: "The CDD was staffed under ACIDS per AR 71-9.", statutoryBasis: "AR 71-9 (29 June 2021, listed ACTIVE) defines ACIDS and references JCIDS — which CJCSI 5123.01J has since replaced with the JFRP."),

        GlossaryTerm(id: "aroc", term: "AROC", definition: "Army Requirements Oversight Council — the Army's senior requirements validation and approval forum, chaired by the Chief of Staff of the Army under AR 71-9 (delegable).", whyItMatters: "With the JFRP returning Service-specific validation to the Services, the AROC's role gains weight. Its published composition predates T2COM/FCC and is pending update.", example: "The CDD went to the AROC for Army validation.", statutoryBasis: "AR 71-9 (29 June 2021). Composition and delegation thresholds predate the 2025-2026 reorganization."),

        GlossaryTerm(id: "crd", term: "CRD", definition: "Capstone Requirements Document — a JFRP document providing top-down, portfolio-level Joint requirements guidance, identifying Joint Force needs without prescribing how Services execute.", whyItMatters: "It is how the reoriented JROC shapes portfolios from the top down under the JFRP.", example: "The portfolio's direction traced to a CRD rather than a system-specific document.", statutoryBasis: "CJCSM 5123.01, Enclosure C (15 January 2026)."),

        GlossaryTerm(id: "jdcr", term: "JDCR", definition: "Joint DOTmLPF-P Change Recommendation — the JFRP mechanism for identifying and approving non-materiel approaches to closing joint capability gaps.", whyItMatters: "It keeps non-materiel solutions visible at the Joint level under the new process.", example: "Rather than new equipment, the gap closed through a JDCR.", statutoryBasis: "CJCSM 5123.01, Enclosure C (15 January 2026)."),

        GlossaryTerm(id: "ccmd-dr", term: "CDR (CCMD Derived Requirement)", definition: "A JFRP document letting a combatant commander formally capture requirements and capability gaps when unmitigated operational risk is unacceptable. A CDR can allow a Service to bypass an ICD and link a CDD directly to the CDR.", whyItMatters: "It gives combatant commands a faster, direct lane into the requirements process.", example: "The CCMD issued a CDR, and the Army linked its CDD to it.", statutoryBasis: "CJCSM 5123.01, Enclosure C (15 January 2026)."),

        GlossaryTerm(id: "con", term: "CoN", definition: "Characteristics of Need — an emerging Army problem-statement construct (used publicly for NGC2 and EMSO efforts) that expresses operational problems and critical capabilities rather than prescriptive requirements.", whyItMatters: "It reflects the shift toward problem statements over specification lists — but it is not yet codified in AR 71-9, so treat it as emerging practice.", example: "Industry responded to the CoN's problem statement instead of a fixed spec."),

        GlossaryTerm(id: "ons", term: "ONS", definition: "Operational Needs Statement — documents an urgent need to correct a high-risk deficiency for current or imminent operations. Per AR 71-9, it is not an ACIDS/JCIDS capability document.", whyItMatters: "It is the urgent lane — distinct from deliberate requirements documents.", example: "The deployed unit submitted an ONS for the immediate gap.", statutoryBasis: "AR 71-9 (29 June 2021)."),

        GlossaryTerm(id: "cns", term: "CNS", definition: "Capability Needs Statement — an Army-specific document for emergent needs tied to anticipated future missions, typically on a roughly two-to-five-year horizon.", whyItMatters: "It fills the space between urgent needs (ONS) and deliberate long-term requirements.", example: "The emerging threat justified a CNS ahead of a full CDD.", statutoryBasis: "AR 71-9 (29 June 2021)."),

        GlossaryTerm(id: "dr", term: "DR", definition: "Directed Requirement — a quick-fire document for transition capabilities that are not expected to be long-term enduring solutions.", whyItMatters: "It lets the Army move on a capability now without committing to it as the permanent answer.", example: "A DR fielded the interim capability while the enduring requirement matured.", statutoryBasis: "AR 71-9 (29 June 2021)."),

        GlossaryTerm(id: "was", term: "WAS", definition: "Warfighting Acquisition System — the November 2025 redesignation of the Defense Acquisition System, prioritizing flexible requirement and resource trades, speed, and accountability.", whyItMatters: "It signals the acquisition system's reorientation around warfighting outcomes and portfolio flexibility.", example: "The program office aligned its strategy to WAS priorities.", statutoryBasis: "Secretary of War memorandum, 'Transforming the Defense Acquisition System into the Warfighting Acquisition System...,' 7 November 2025.")
    ]
